...
4. WebKit activation is retried on `pointerdown`, `pointerup`, `touchstart`, `touchend`, `click`, and `keydown`. Do not deduplicate retries across different trusted events.
5. Each trusted retry starts a one-sample silent output graph and calls `resume()`. This handles Safari/WKWebView versions that reject the first down/start event but accept end/click.
6. Fetching/decoding the phase sound must never be awaited by the opening curtain.
7. If Web Audio is unavailable, the two fixed HTML audio elements are the same-moment fallback. Never replay a missed cue seconds later.
8. Dematerialize and rematerialize both use the attached punch sound at normal pitch. Do not alter BGM pitch or the music mix to emulate this effect.
...
- `<html data-phase-audio-unlock-attempts>` and `data-phase-audio-prime`.
...
- The next release must preserve PR #17's loader decoupling while removing its gameplay deadlock.
